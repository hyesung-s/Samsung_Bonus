(() => {
  ...
  const STORAGE_KEY = "samsung-bonus-calculator-state-v15-clean";
  ...
  function buildReceiptForDivision(divisionResult, salary, stockVwapPriceWon, stockClosePriceWon) {
    ...
    return { opi1Tax, opi2Tax, totalBonusTax, stockGrant, finalReceipt };
  }
  ...
  function renderComparison(result) {
    ...
    const allMetrics = RESULT_DIVISION_IDS.flatMap((id) => {
      ...
      return [item.opi1, item.opi2Sector, item.opi2Business, item.opi2Total, item.totalBonus, receipt.totalBonusTax.netPayment, receipt.finalReceipt];
    });
