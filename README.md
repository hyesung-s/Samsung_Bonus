...
- 개인 조건 입력 영역을 최상단으로 이동
- 본인 연봉, 본인 사업부, 평균연봉을 한 줄 입력 UI로 구성
- 요약 카드에 OPI1/OPI2 금액과 연봉 대비 비율 표시
- OPI2 균등분배 명칭을 `부문`으로 변경
- OPI2 사업부별 분배 명칭을 `사업부`로 변경
- 사업부별 가정과 공통 OPI2 설정을 하나의 표로 통합
- 공통 행의 영업이익 칸에 `제원 공유 사업부` 선택 기능 추가
- 공통 행의 OPI2 재원 칸에 `가중치(배분율)` 입력 기능 추가
- 모바일에서는 표가 카드형으로 변환되도록 반응형 레이아웃 적용
...
- 공통 가중인원 = 공통 인원 × 공통 가중치
- 부문 가중인원 = 메모리 인원 + 파운드리 인원 + S.LSI 인원 + 공통 가중인원
- OPI2 부문 = OPI2 부문 재원 ÷ 부문 가중인원
- 사업부 배분에서는 사용자가 선택한 사업부의 배분 대상 인원에 공통 가중인원을 더합니다.
- 공통 OPI2 사업부 = 선택 사업부 기준금액 × 공통 가중치
...
git init
git add .
git commit -m "Update Samsung bonus calculator"
git branch -M main
